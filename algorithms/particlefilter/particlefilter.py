...
class Model(interfaces.ParticleFiltering):
    ...
    def update(self, particles, u, t, noise):
        ...
        speed = noise[:, 1] + self.human_speed
    ...
    def measure(self, particles, y, t):
        ...
        for k in range(len(particles)):
            ...
            dist = numpy.linalg.norm(particle[:2] - y) + 0.1
            # print("#########")
            # print(dist)
            # weight = (1/dist * 1000)-1.0
            # print(weight)
            # print(kalman.lognormpdf(weight, self.R))
...
def AppendData(timestamp, loc, particles=[]):
    obj = [timestamp, loc, particles]
...
def init_particlefilter(timestamp, init_loc):
    ...
    num = 1000
    P0 = 100.0
    human_speed = 0.1 * 100.0
    human_heading_change = 45
    Q = numpy.asarray((human_heading_change, human_speed * 0.5))  # heading, speed variances
    R = numpy.asarray(((20.0,),))
    # R = numpy.asarray(((0 ** 2,),))
    ...
    sim.pt = filter.ParticleTrajectory(sim.model, num, resample=2.0 / 3.0)
    # # result_history = []
    # for i in range(1000):
...
def update_particlefilter(timestamp, ble_predict):
    ...
    threading.Thread(target=AppendData, args=(timestamp, meanXY, particles)).start()
...
def Update(updateRequest):
    ...
    ble_predict = numpy.array(updateRequest.BlePredict)
    print("Update: ", timestamp)
    return update_particlefilter(timestamp, ble_predict), True
