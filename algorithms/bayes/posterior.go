...
//
//	if gp == nil{
//		gp = dbm.GM.GetGroup(res.Group)
//		//glb.Debug.Println(gp)
//		//defer dbm.GM.GetGroup(res.Group).Set(gp)
//		// No need to write back
//	}
//
//	mdVals := gp.Get_MiddleData_Val()
//	bayesPriors := gp.Get_AlgoData().Get_BayesPriors()
//
//	macs := []string{}
...
//	}
//	n, inNetworkAlready := clustring.HasNetwork(mdVals.NetworkMacs, macs)
//	//Debug.Println(ps.NetworkMacs)
//	//Debug.Println(macs)
//	//Debug.Println(n, inNetworkAlready, ps.NetworkLocs[n])
//	if !inNetworkAlready {
//		glb.Warning.Println("Not in network")
//		glb.Debug.Println(n, inNetworkAlready, mdVals.NetworkLocs[n], res)
//	}
//
//	if len(mdVals.NetworkLocs[n]) == 1 {
//		for key := range mdVals.NetworkLocs[n] {
...
//	PA := 1.0 / float64(len(mdVals.NetworkLocs[n]))
//	PnA := (float64(len(mdVals.NetworkLocs[n])) - 1.0) / float64(len(mdVals.NetworkLocs[n]))
//	for loc := range mdVals.NetworkLocs[n] {
//		PBayes1[loc] = float64(0)
//		PBayes2[loc] = float64(0)
//		for mac := range W {
//			weight := float64(0)
//			nweight := float64(0)
//			if _, ok := bayesPriors[n].MacFreq[loc][mac]; ok {
//				weight = float64(bayesPriors[n].MacFreq[loc][mac])
//			} else {
//				weight = float64(bayesPriors[n].Special["MacFreqMin"])
//			}
//			if _, ok := bayesPriors[n].NMacFreq[loc][mac]; ok {
//				nweight = float64(bayesPriors[n].NMacFreq[loc][mac])
//			} else {
//				nweight = float64(bayesPriors[n].Special["NMacFreqMin"])
//			}
//			PBayes1[loc] += math.Log(weight*PA) - math.Log(weight*PA+PnA*nweight)
//
//			if float64(mdVals.MacVariability[mac]) >= bayesPriors[n].Special["VarabilityCutoff"] && W[mac] > glb.MinRssiOpt {
//				ind := int(W[mac] - glb.MinRssi)
//				if len(bayesPriors[n].P[loc][mac]) > 0 {
//					PBA := float64(bayesPriors[n].P[loc][mac][ind])
//					//PBnA := float64(ps.bayesPriors[n].NP[loc][mac][ind])
//					if PBA > 0 {
...
//		PBayesMix[key] = bayesPriors[n].Special["MixIn"]*PBayes1[key] + (1-bayesPriors[n].Special["MixIn"])*PBayes2[key]
...
//	//#cache
//	PA := 1.0 / float64(len(mdVals.NetworkLocs[n]))//the real prior !
//	PnA := (float64(len(mdVals.NetworkLocs[n])) - 1.0) / float64(len(mdVals.NetworkLocs[n])) // 1.0 - PA
//
...
//			nweight := float64(0)
//
//			// todo: The condition should be like this: ok && ps.bayesPriors[n].MacFreq[loc][mac]!=0
//			//if _, ok := ps.bayesPriors[n].MacFreq[loc][mac]; ok {
//			//#cache
//			if _, ok := bayesPriors[n].MacFreq[loc][mac]; ok {
//				//if the MacFreq (or weight) of a Mac in a location is high, it means that the location is near the AP(Mac).
//				weight = float64(bayesPriors[n].MacFreq[loc][mac])
//			} else {
//				//todo:why not using 0 instead of ps.bayesPriors[n].Special["MacFreqMin"]?
//				weight = float64(bayesPriors[n].Special["MacFreqMin"])
//			}
//			if _, ok := bayesPriors[n].NMacFreq[loc][mac]; ok {
...
//			if float64(mdVals.MacVariability[mac]) >= cutoff && resRoutes[mac] > glb.MinRssiOpt { //TODO: why calculating the mac variability of a mac not a location?
//				ind := int(resRoutes[mac] - glb.MinRssi) //same as what is done in P calculation
//				if len(bayesPriors[n].P[loc][mac]) > 0 {
//					PBA := float64(bayesPriors[n].P[loc][mac][ind])
//					PBnA := float64(bayesPriors[n].NP[loc][mac][ind])
//					if PBA > 0 {
